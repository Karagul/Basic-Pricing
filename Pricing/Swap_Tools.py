# -*- coding: utf-8 -*-
"""
Created on Sun Nov  4 12:10:36 2018
Tools functions for swap pricing and fixed rate finding problems
@author: shaolun du
...
def KRD_regroup(KRDs):
    """ Regrouping KRDs into key years
        Given key years: 1,2,3,5,7,10,15,20,25,30,40
        Current given krds by years from 1 to 40
    """
    year_loc = [1,2,3,5,7,10,15,20,25,30,40]
    Ans = {yy:0 for yy in year_loc}
    for loc,val in enumerate(KRDs):
        t_loc = loc+1
        for lloc in range(1,len(year_loc)):
            # Find current location
            if t_loc >= year_loc[lloc-1] and t_loc < year_loc[lloc]:
                delta = val*(1.0-(t_loc-year_loc[lloc-1])/(year_loc[lloc]-year_loc[lloc-1]))
                Ans[year_loc[lloc-1]] += delta
                Ans[year_loc[lloc]]   += (val-delta)
    return Ans
...
def augument_by_frequency( input_list, 
    ...
    time_frequency = relativedelta( months = int(months) )
    next_date = cur_date
...
def get_fwd_rate( Rates_Curve, 
    ...
    if fwd_period < 0:
        ...
        print(currency,fwd_period,cur_time)
    ...
    """
    flag = False
    if check_if_last_day_of_month(cur_time):
        flag = True
    ...
    if flag:
        fwd_time = last_day_of_month(fwd_time)
...
def interpolation_norm( cur_time, 
                        start_time, 
                        start_value, 
                        end_time, 
                        end_value ):
...
def group_PMT_by_frequency( CF_table, 
                            frequency = "A" ):
    '''
        Grouping convention:
            ...
            N       nanoseconds
    '''
    """ Inputs should be a data frame 
        with columns = ["Date","PMT"]
        C F_table = CF_table.groupby("Date")[["Date","PMT"]].sum()
    """
    CF_table = CF_table.groupby("Date").sum()
    CF_table.index = pd.to_datetime(CF_table.index)
    CF_table = CF_table.resample(frequency).sum()
    return CF_table
...
def str_2_date( sdate ):
    if isinstance( sdate, str ):
        for fmt in ( "%Y-%m-%d", "%m/%d/%Y" ):
            try:
                return dt.datetime.strptime( sdate, fmt ).date()
            except ValueError:
                pass
    else:
        return sdate
